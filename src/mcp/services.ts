  ...
  return {
    ...
    domainContext: {
      ...
      nodeTypes: {
        ...
        RouteDefinition: {
          description:
            'Explicit route definitions from route files. CRITICAL: Individual route details (method, path, authenticated, handler, controllerName) are stored in the "context" property as a JSON string.',
          purpose: 'Map HTTP paths and methods to controller handlers',
          commonProperties: ['name', 'context', 'filePath', 'sourceCode'],
          contextStructure:
            'The context property contains JSON with structure: {"routes": [{"method": "POST", "path": "/v1/endpoint", "controllerName": "SomeController", "handler": "methodName", "authenticated": true}]}',
          parsingInstructions:
            'To get individual routes: (1) Parse JSON with apoc.convert.fromJsonMap(rd.context) (2) UNWIND the routes array (3) Access route.method, route.path, route.handler, route.authenticated, route.controllerName',
          exampleQuery:
            'MATCH (rd:RouteDefinition) WITH rd, apoc.convert.fromJsonMap(rd.context) AS ctx UNWIND ctx.routes AS route RETURN route.method, route.path, route.controllerName, route.handler, route.authenticated ORDER BY route.path',
        },
        HttpEndpoint: {
          ...
          commonProperties: ['name', 'filePath', 'sourceCode'],
          exampleQuery:
            'MATCH (e:HttpEndpoint)<-[r:ROUTES_TO_HANDLER]-(rd) WHERE apoc.convert.fromJsonMap(r.context).authenticated = true RETURN e.name, apoc.convert.fromJsonMap(r.context).path as path',
        },
      },
      ...
      relationships: {
        ...
        ROUTES_TO_HANDLER: {
          ...
          contextNote:
            'IMPORTANT: context is stored as a JSON string. Access properties using apoc.convert.fromJsonMap(r.context).propertyName',
      ...
      commonQueryPatterns: [
        ...
        },
        {
          ...
          query:
            'MATCH (rd:RouteDefinition)-[r:ROUTES_TO_HANDLER]->(m) WHERE apoc.convert.fromJsonMap(r.context).authenticated = true RETURN apoc.convert.fromJsonMap(r.context).path as path, apoc.convert.fromJsonMap(r.context).method as method, m.name',
        },
        {
        ...
        },
        {
          ...
          query:
            'MATCH path = (c:Controller)-[:INJECTS*1..3]->(r:Repository)-[:USES_DAL]->(d:DAL) WHERE c.name = $controllerName RETURN [n in nodes(path) | n.name] as executionPath',
        },
      ],
    },
  };
